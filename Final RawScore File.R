...
#ACTUALLY DONT
#Rearing1
m2.5 <- MCMCglmm(RawScore ~ 1 + sex + Rearing, random = ~animal + ID + study2, family = "gaussian",
                prior = prior3, pedigree = p, data = D, nitt = 500000, verbose = F, burnin = 100000, thin = 250)
summary(m2.5)
#Rearing2
#m3.5 <- UMCMCglmm(RawScore ~ 1 + sex + Rearing2, random = ~animal + ID + study2, family = "gaussian",
                #prior = prior3, pedigree = p2, data = D, nitt = 500000, verbose = F, burnin = 100000, thin = 250)
#Rearing3
#m4.5 <- UMCMCglmm(RawScore ~ 1 + sex + Rearing3, random = ~animal + ID + study2, family = "gaussian",
                #prior = prior3, pedigree = p2, data = D, nitt = 500000, verbose = F, burnin = 100000, thin = 250)
#summary(m2.5) ; summary(m3.5) ; summary(m4.5)
...
plot(m2.5$Sol); plot(m2.5$VCV)
plot(m3$Sol); plot(m3$VCV)
plot(m3.5$Sol); plot(m3.5$VCV)
plot(m4$Sol); plot(m4$VCV)
plot(m4.5$Sol); plot(m4.5$VCV)
