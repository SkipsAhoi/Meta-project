...
#The Priors
prior2 <- list(R = list(V=1, nu=0.02), G = list(G1 = list(V=1, nu=0.02), G2 = list(V=1, nu=0.002)))
prior3 <- list(R = list(V=1, nu=0.02), G = list(G1 = list(V=1, nu=0.02), G2 = list(V=1, nu=0.02), G3 = list(V=1, nu=0.02)))

#The Models. Outcome = SLBinary
m1 <- MCMCglmm(SLBinary ~ 1, random = ~animal + ID + study, family = "categorical",
               prior = prior3, pedigree = p2, verbose = FALSE, data = D, nitt = 100000, burnin = 10000, thin = 100)
m2 <- MCMCglmm(SLBinary ~ 1 + sex, random = ~animal + ID + study, family = "categorical",
               prior = prior3, pedigree = p2, data = D, verbose = FALSE, nitt = 100000, burnin = 10000, thin = 100)
m3 <- MCMCglmm(SLBinary ~ 1 + sex + Rearing, random = ~animal + ID + study, family = "categorical",
               prior = prior3, pedigree = p2, data = D, nitt = 100000, verbose = F, burnin = 10000, thin = 100)
m4 <- MCMCglmm(SLBinary ~ 1 + sex + Rearing + Age, random = ~animal + ID, family = "categorical",
               prior = prior3, pedigree = p2, data = D, nitt = 100000, verbose = F, burnin = 10000, thin = 100)
m5 <- MCMCglmm(SLBinary ~ 1 + sex + Rearing + Age + CurExp, random = ~animal + ID + study, family = "categorical",
               prior = prior3, pedigree = p2, data = D, nitt = 500000, verbose = F, burnin = 100000, thin = 200)
summary(m5)
dredgeSLBModel<-dredge(m5, rank="DIC"); dredgeModel
dredgeModel
avgDredge<-model.avg(dredgeModel); avgDredge
mavg<-model.avg(dredgeModel);mavg
model.sel(dredgeModel)
summary(mavg)
confint(mavg, full = T)
HPDinterval(m5)
#Diagnostics
...
#Inference
#Model Comparison
ms<-model.sel(m1, m2, m3, m4, m5, rank = DIC); ms
ms2<-model.sel(m1, m2, m3, m4, m5, m6, m7, m8, m9, m10, rank = DIC); ms2
summary(m5)

#phenotypic variance conditional on fixed effects
mVP<-m5$VCV[,"animal"]+m5$VCV[,"ID"]+m5$VCV[,"units"]
#between individual variance
mIP<-m5$VCV[,"animal"]+m5$VCV[,"ID"]
#repeatability
...
m6 <- MCMCglmm(SLBinary ~ 1, random = ~animal + ID + Context, family = "categorical",
               prior = prior3, pedigree = p2, verbose = FALSE, data = D, nitt = 100000, burnin = 10000, thin = 100)
m7 <- MCMCglmm(SLBinary ~ 1 + sex, random = ~animal + ID + Context, family = "categorical",
               prior = prior3, pedigree = p2, data = D, verbose = FALSE, nitt = 100000, burnin = 10000, thin = 100)
m8 <- MCMCglmm(SLBinary ~ 1 + sex + Rearing, random = ~animal + ID + Context, family = "categorical",
               prior = prior3, pedigree = p2, data = D, nitt = 100000, verbose = F, burnin = 10000, thin = 100)
m9 <- MCMCglmm(SLBinary ~ 1 + sex + Rearing + Age, random = ~animal + ID + Context, family = "categorical",
               prior = prior3, pedigree = p2, data = D, nitt = 100000, verbose = F, burnin = 10000, thin = 100)
m10 <- MCMCglmm(SLBinary ~ 1 + sex + Rearing + Age + CurExp, random = ~animal + ID+ Context, family = "categorical",
                prior = prior3, pedigree = p2, data = D, nitt = 100000, verbose = F, burnin = 10000, thin = 100)

#with Study instead of Task
m6 <- MCMCglmm(SLBinary ~ 1, random = ~animal + ID + study, family = "categorical",
               prior = prior3, pedigree = p2, verbose = FALSE, data = D, nitt = 100000, burnin = 10000, thin = 100)
m7 <- MCMCglmm(SLBinary ~ 1 + sex, random = ~animal + ID + study, family = "categorical",
               prior = prior3, pedigree = p2, data = D, verbose = FALSE, nitt = 100000, burnin = 10000, thin = 100)
m8 <- MCMCglmm(SLBinary ~ 1 + sex + Rearing, random = ~animal + ID + study, family = "categorical",
               prior = prior3, pedigree = p2, data = D, nitt = 100000, verbose = F, burnin = 10000, thin = 100)
m9 <- MCMCglmm(SLBinary ~ 1 + sex + Rearing + Age, random = ~animal + ID + study, family = "categorical",
               prior = prior3, pedigree = p2, data = D, nitt = 100000, verbose = F, burnin = 10000, thin = 100)
m10 <- MCMCglmm(SLBinary ~ 1 + sex + Rearing + Age + CurExp, random = ~animal + ID+ study, family = "categorical",
                prior = prior3, pedigree = p2, data = D, nitt = 100000, verbose = F, burnin = 10000, thin = 100)